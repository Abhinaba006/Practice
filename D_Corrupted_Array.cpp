...
void solve(){
    ...
    new1(n);
    Arr(a, n+2);
    ll sum = accumulate(a, a+n+2, 0LL);
    multiset<ll> st;
    rep(i, 0, n+2) st.insert(a[i]*2);
    rep(i, 0, n+2){
        if(st.find((sum-a[i]))!=st.end()){
            st.erase(st.lower_bound(a[i]*2));
            if(st.find(sum-a[i])!=st.end())  st.erase(st.lower_bound((sum-a[i])));
            else{
                st.insert(a[i]*2);
                continue;
            }
            for(auto c:st) cout<<c/2<<" ";
            cout<<endl;
            return;
        }
    }
