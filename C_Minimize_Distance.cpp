...
void solve(){
    ...
    Arr(a, n);
    vi pos, neg;
    rep(i, 0, n) if(a[i]>=0) pos.pb(a[i]); else neg.pb(a[i]);
    ...
    for(int i=neg.size()-1;i>=0;i-=k) ans += abs(neg[i]);
    // cout<<"HI"<<endl;
    cout<<2*ans-max(pos.size()?pos.back():0, neg.size()?abs(neg.back()):0)<<endl;
}
