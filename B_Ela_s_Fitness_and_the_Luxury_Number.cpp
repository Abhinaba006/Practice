...
#define new2(n, k) ll n, k;cin >> n >> k;
...
void solve(){
    new2(l, r);
    ll rl = sqrt(l), rr = sqrt(r);
    ll rl1 = sqrt(l), rr1 = sqrt(r);
    ll ans=0;
    ll sq = rl*rl, nxtSq = sq+(2*rl)+1;
    while(rl<=rr){
        ll t;
        if(sq<l && nxtSq>r+1) t = r-l;
        else if(sq<l) t = nxtSq - l;
        else if(nxtSq>r+1) t = r+1 - sq;
        else t = nxtSq - sq;
        cout<<t<<" ,";
        sq = nxtSq;
        nxtSq = sq+(2*rl)+1;
